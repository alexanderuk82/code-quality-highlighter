...
export class PersonalizedFixGenerator {
  ...
  public generateFix(match: PatternMatch, context: MatchContext): FixSuggestion | undefined {
    try {
      ...
      const fix = this.generateFixForPattern(
        match.ruleId,
        problemCode,
        fileContext,
        match,
  ...
  private generateFixForPattern(
    ...
    console.log(`[PersonalizedFix] Problem code length: ${problemCode.length}`);
    console.log(`[PersonalizedFix] Problem code snippet:`, problemCode.substring(0, 100));
    console.log(`[PersonalizedFix] File context:`, { isReact: fileContext.isReact, isNodeJS: fileContext.isNodeJS });
   ...
   */
  private fixMemoryLeak(problemCode: string, _fileContext: FileContext, context: MatchContext): FixSuggestion {
...
// Call cleanup when appropriate (page unload, component destroy, etc.)`;
   ...
   */
  private fixNestedLoops(problemCode: string, _fileContext: FileContext, _context: MatchContext): FixSuggestion {
...
for (const item of ${innerArray}) {
   ...
   */
  private fixStringConcatenation(problemCode: string, _fileContext: FileContext, context: MatchContext): FixSuggestion {
...
const ${variable} = ${variable}Parts.join('');`;
   ...
   */
  private fixBlockingOperations(problemCode: string, _fileContext: FileContext, _context: MatchContext): FixSuggestion {
...
loadData().then(${variable} => {
   ...
   */
  private fixLongFunction(problemCode: string, _fileContext: FileContext, _context: MatchContext): FixSuggestion {
...
async function ${funcName}(params) {
   ...
   */
  private fixMissingMemo(problemCode: string, fileContext: FileContext, _context: MatchContext): FixSuggestion {
...
export default ${componentName};`;
   ...
   */
  private fixConsoleUsage(problemCode: string, _fileContext: FileContext, _context: MatchContext): FixSuggestion {
...
// ${problemCode} // <-- Remove this line`;
   ...
   */
  private fixExpensiveOperations(problemCode: string, _fileContext: FileContext, _context: MatchContext): FixSuggestion {
...
for (const item of otherArray) {
   ...
   */
  private fixDOMQueries(problemCode: string, _fileContext: FileContext, _context: MatchContext): FixSuggestion {
    // Extract the query
...
for (const item of items) {
   ...
   */
  private fixInlineFunctions(problemCode: string, fileContext: FileContext, _context: MatchContext): FixSuggestion {
    if (!fileContext.isReact) {
      return { type: 'copy', text: '// Not applicable to non-React code' };
    }
...
<Component ${propName}={handle${this.capitalize(propName || 'Click')}} />`;
   ...
   */
  private fixStateMutation(problemCode: string, fileContext: FileContext, _context: MatchContext): FixSuggestion {
    if (!fileContext.isReact) {
      return { type: 'copy', text: '// Not applicable to non-React code' };
    }
...
set${this.capitalize(stateName || 'State')}(prev => {
   ...
   */
  private fixMissingKeys(problemCode: string, fileContext: FileContext, _context: MatchContext): FixSuggestion {
    if (!fileContext.isReact) {
      return { type: 'copy', text: '// Not applicable to non-React code' };
    }
...
${arrayName}.map((${param}, index) => (
   ...
   */
  private createGenericFix(problemCode: string, ruleId: string): FixSuggestion {
    return {
      type: 'copy',
      text: `// YOUR CODE:
${problemCode}

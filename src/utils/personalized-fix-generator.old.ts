 ...
 */
export class PersonalizedFixGenerator {
  private static instance: PersonalizedFixGenerator;

  private constructor() {}

  public static getInstance(): PersonalizedFixGenerator {
   ...
   */
  public generateFix(match: PatternMatch, context: MatchContext): FixSuggestion | undefined {
    try {
      ...
      const personalizedFix = this.buildPersonalizedFix(
        match.ruleId,
        codeElements,
        fileContext,
        match,
   ...
   */
  private getExactProblemCode(match: PatternMatch, context: MatchContext): string {
    try {
   ...
   */
  private detectFileContext(context: MatchContext): FileContext {
   ...
   */
  private extractCodeElements(match: PatternMatch, context: MatchContext): CodeElements {
    const node = match.node;
    const sourceCode = context.sourceCode;
    
    // Extract based on node type
    const elements: CodeElements = {
...
const ${variable} = ${variable}Parts.join('');`;
   ...
   */
  private fixMissingMemo(elements: CodeElements, fileContext: FileContext): FixSuggestion {
...
export default ${componentName};`;
   ...
   */
  private fixLongFunction(elements: CodeElements, _fileContext: FileContext): FixSuggestion {
...
async function performOperation() {
   ...
   */
  private fixExpensiveLoops(elements: CodeElements, _fileContext: FileContext): FixSuggestion {
    const loop = elements.loopInfo?.[0];
    const operation = elements.methodCalls[0];
...
for (const ${iteratorVar} of ${arrayName}) {
   ...
   */
  private fixInlineFunctions(elements: CodeElements, fileContext: FileContext): FixSuggestion {
    if (!fileContext.isReact) {
      return { type: 'copy', text: '// Not applicable to non-React code' };
    }
...
const ${componentName} = () => {
   ...
   */
  private fixMissingKeys(elements: CodeElements, fileContext: FileContext): FixSuggestion {
    if (!fileContext.isReact) {
      return { type: 'copy', text: '// Not applicable to non-React code' };
    }
...
{${arrayName}.map((${iteratorVar}, index) => (
   ...
   */
  private fixStateMutation(elements: CodeElements, fileContext: FileContext): FixSuggestion {
    if (!fileContext.isReact) {
      return { type: 'copy', text: '// Not applicable to non-React code' };
    }
...
// cleanup${this.capitalize(elementVar)}();`;
   ...
   */
  private extractFunctionContext(match: PatternMatch, context: MatchContext): any {
    const sourceCode = context.sourceCode;
   ...
   */
  private buildGenericMemoryLeakFix(): FixSuggestion {
    return {
      type: 'copy',
...
// Call cleanup when appropriate`
   ...
   */
  private fixConsoleUsage(): FixSuggestion {
...
logger.info('Your log message');`;
   ...
   */
  private filterByContext(fix: FixSuggestion, context: FileContext): FixSuggestion {
  ...
  // Helper methods
  private extractVariables(node: AnyASTNode, sourceCode: string): string[] {
  ...
  private extractFunctions(node: AnyASTNode, sourceCode: string): string[] {
  ...
  private extractClasses(node: AnyASTNode, sourceCode: string): string[] {
  ...
  private extractImports(sourceCode: string): string[] {
    ...
    while ((match = importPattern.exec(sourceCode)) !== null) {
      if (match[3]) imports.push(match[3]); // Package name
    }
    
    return imports;
  }

  private extractComponentName(node: AnyASTNode, sourceCode: string): string | undefined {
    const nodeCode = this.getNodeCode(node, sourceCode);
  ...
  private extractLoopInfo(node: AnyASTNode, sourceCode: string): LoopInfo[] {
  ...
  private extractMethodCalls(node: AnyASTNode, sourceCode: string): string[] {
  ...
  private extractParameters(node: AnyASTNode, sourceCode: string): string[] {
    const nodeCode = this.getNodeCode(node, sourceCode);
  ...
  private extractProperties(node: AnyASTNode, sourceCode: string): string[] {
  ...
  private getNodeCode(node: AnyASTNode, sourceCode: string): string {
    if (node.start !== undefined && node.end !== undefined) {
      return sourceCode.substring(node.start, node.end);
    }
    return '';
  ...
  private buildGenericFix(): FixSuggestion {
    return {
      type: 'copy',
...
interface LoopInfo {
  type: 'for' | 'for-of' | 'for-in' | 'forEach' | 'while' | 'do-while';
  iteratorVar?: string;
  indexVar?: string;
  arrayName?: string;
}
...
export const personalizedFixGenerator = PersonalizedFixGenerator.getInstance();

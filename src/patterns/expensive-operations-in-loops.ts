 ...
 */
export class ExpensiveOperationsInLoopsMatcher implements PatternMatcher {
  ...
  public match(node: AnyASTNode, context: MatchContext): boolean {
    ...
    if (this.isExpensiveArrayMethod(node)) return true;

    // Check for DOM operations
    if (this.isExpensiveDOMOperation(node)) return true;

    // Check for expensive object operations
    if (this.isExpensiveObjectOperation(node)) return true;

    // Check for repeated function calls
    if (this.isRepeatedFunctionCall(node)) return true;
  ...
  private estimateComplexity(_operationType: string, methodName: string): number {
  ...
  private getSuggestion(operationType: string, _methodName: string): string {
 ...
 */
const expensiveOperationsInLoopsTemplate: TooltipTemplate = {
 ...
 */
export const expensiveOperationsInLoopsRule: PatternRule = {
